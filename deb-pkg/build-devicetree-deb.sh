...
DTDEB_OUTNAME="devicetrees-citadel-os-quartz64-$DTDEB_VERSION-$DTDEB_REVISION_$DTDEB_ARCH.deb"

# Download quartz64-deploy branch archive
rm -f linux-quartz64-deploy.zip
rm -rf linux-quartz64-deploy
wget -q -O linux-quartz64-deploy.zip https://github.com/AarpnDewes/linux/archive/refs/heads/path-1.zip
...
rm -rf data/
mkdir -p data/usr/lib/devicetrees-citadel-os-quartz64/rockchip/
install --mode=644 linux-quartz64-deploy/arch/arm64/boot/dts/rockchip/rk3566-quartz64-a.dtb \
    data/usr/lib/devicetrees-citadel-os-quartz64/rockchip/
